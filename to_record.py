...
def rezie_all():
    img_path = gb.glob("CamVid\\test\\*.jpg") 
    for path in img_path:
        background = Image.open(path)
        background = background.resize((480,360))
        background.save(path,quality=95,subsampling=0)

def bytes_feature(values):
    #return tf.train.Feature(bytes_list=tf.train.BytesList(value=[values]))
    return tf.train.Feature(bytes_list=tf.train.BytesList(value=[values.encode('utf-8') if type(values)==str else values]))
        ...
        with tf.python_io.TFRecordWriter(output_tfrecord_filename) as tfrecord_writer:
            ...
            for i in range(start_ndx, end_ndx):
                try:
                    ...
                    # print(anno_data)
                    example = image_to_example(raw_img_data, anno_data, os.path.basename(img_filenames[i]), height, width)
